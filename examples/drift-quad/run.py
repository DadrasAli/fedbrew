...
``configs/examples/drift-quad/``, run by the ``fedbrew`` CLI as a subprocess,
against data written by ``fedbrew generate`` from
``data/configs/examples/drift-quad.yaml``. Nothing in this file composes a
...
Usage
-----
    python examples/drift-quad/run.py                       # the base dials
    python examples/drift-quad/run.py --setting drift-quad-rate
    python examples/drift-quad/run.py --arm scaffold
    python examples/drift-quad/run.py --table-only          # re-table outputs/
"""
...
#: that writes the data each reads.
SETTINGS = ("drift-quad", "drift-quad-rate", "drift-quad-floor")

#: The order the README's tables use: the mechanism each arm brings, roughly
...
def summarise(output_dir: Path) -> dict[str, Any] | None:
    ...
    return {
        ...
        "median_tail": median,
        "best": min(gaps),
        "first_below_1e2": _first_below(gaps, 1e-2),
        ...
        "fit_distance_to_client_optimum": _float(final, "fit_distance_to_client_optimum"),
...
COLUMNS = (
    ...
    ("best", "best gap", "{:.2e}"),
    ("first_below_1e2", "first < 1e-2", "{}"),
    ("first_below_1e6", "first < 1e-6", "{}"),
    ("first_below_1e12", "first < 1e-12", "{}"),
    ("fit_distance_to_optimum", "fit dist to x*", "{:.2e}"),
    ("fit_distance_to_client_optimum", "fit dist to x_i*", "{:.2e}"),
